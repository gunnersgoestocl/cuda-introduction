...
void output_timeline_data(const TimingInfo& global_timing, const TimingInfo& shared_timing) {
    ...
    float global_copy_in_end = global_start + global_timing.copy_in_time * 1000;
    ...
    // 共有メモリ版のタイムライン（グローバル版の終了後に開始）
    float shared_start = global_copy_out_end + 50; // 50ms間隔
    float shared_copy_in_end = shared_start + shared_timing.copy_in_time * 1000;
